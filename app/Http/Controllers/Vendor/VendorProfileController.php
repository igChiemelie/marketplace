...
class VendorProfileController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('shop_logo')) {
            ...
            Storage::putFileAs('public/vendors/logos', $logoFile, basename($logoPath));
        ...
        if ($request->hasFile('shop_banner')) {
            if ($vendor->shop_banner && Storage::exists($vendor->shop_banner)) {
                Storage::delete($vendor->shop_banner);
            }

            $bannerFile = $request->file('shop_banner');
            $bannerPath = 'vendors/banners/' . uniqid() . '.' . $bannerFile->getClientOriginalExtension();
            Storage::putFileAs('public/vendors/banners', $bannerFile, basename($bannerPath));
